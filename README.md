...
顶部导航栏的 **Ask AI** 提供一段可复制提示词，帮助支持网页访问的外部 Agent 按需读取 PianoAI 已发布的曲谱、静态分析和参考演绎记录。提示词要求 Agent 区分 PianoAI 数据、自身推断和其他来源，并在 PianoAI 证据不足时明确说明。
...
浏览器打开 Vite 输出的 `Local` 地址。开发模式会优先读取本地参考音频；本地文件不存在时，再从已配置的 R2 地址加载。普通使用和前端开发不需要安装 Python、CUDA 或转录模型。
