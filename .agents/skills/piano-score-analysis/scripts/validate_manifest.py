#!/usr/bin/env python3
"""Validate every PianoAI analysis referenced by the public manifest."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--manifest", type=Path, default=Path("public/analysis/manifest.json"))
    parser.add_argument("--schema", type=Path, default=Path("analysis/schema/score-analysis.schema.json"))
    parser.add_argument("--library-dir", type=Path, default=Path("public/musicxml"))
...
def main() -> int:
    ...
    for index, item in enumerate(items):
        ...
        analysis_file = item.get("analysisFile")
        source_hash = item.get("sourceHash")
        if not all(isinstance(value, str) and value for value in (score_id, source_file, analysis_file, source_hash)):
            errors.append(f"{prefix}: missing scoreId, fileName, analysisFile, or sourceHash")
            continue
        for value, seen, field in (
            (score_id, seen_score_ids, "scoreId"),
            (source_file, seen_source_files, "fileName"),
            (analysis_file, seen_analysis_files, "analysisFile"),
        ):
            if value in seen:
        ...
        try:
            ...
            analysis_path = safe_child(args.manifest.parent, analysis_file)
